- [DB + API architecture](db-api-architecture.md) — full-stack: Express on :8000, Vite proxy /api, DataContext seeds from static then replaces with DB; changing seed data requires `npx tsx server/seed.ts --force` to push changes into the DB
...
- [Operator Tablet modal](operator-tablet-modal.md) — tablet lives in a sidebar popup button, not a nav item; inModal prop + shell() helper + display:contents wrappers pattern
